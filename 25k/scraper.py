...
INVALID_EXTENSIONS = {
    ...
    # Special/Scientific
    'lif', 'mat', 'dat', 'rdata', 'rds',  # ← Added lif and data files
...
def is_valid(url):
    """
    Comprehensive URL validation with STRICT domain checking
    ...
    try:
        ...
        # Must be EXACTLY one of the valid domains or a subdomain of them
        is_valid_domain = False
        
        for domain in VALID_DOMAINS:
            # Check if it's exactly the domain OR a subdomain of it
        ...
        # 3. EXPLICITLY block non-ICS/CS/Informatics/Stat domains
        # Even if they somehow passed the check above
        blocked_domains = [
            'physics.uci.edu',
            'economics.uci.edu',
            'chem.uci.edu',
        ...
        for blocked in blocked_domains:
            if netloc == blocked or netloc.endswith('.' + blocked):
                log_rejection(f"blocked_domain_{blocked.replace('.uci.edu', '')}", url)
                return False
        ...
        # 8. File extensions (path-based)
        if "." in path_lower:
            parts = path_lower.rsplit('.', 1)
            if len(parts) == 2:
                ext = parts[1].split('/')[0].split('?')[0]
                if ext not in ALLOWED_EXTENSIONS and ext in INVALID_EXTENSIONS:
                    log_rejection(f"extension_{ext}", url, save_sample=False)
                    return False
        
        # 9. Query parameter format (e.g., ?format=txt)
        if parsed.query:
            query_lower = parsed.query.lower()
            
            # Block format parameters
            format_blocks = ['format=txt', 'format=pdf', 'format=csv', 
                           'export=txt', 'export=pdf', 'download=']
            
            for block in format_blocks:
                if block in query_lower:
                    log_rejection("format_parameter", url, save_sample=False)
                    return False
        
        # 10. Search/print/share endpoints
